...
public class KanBagis {
    ...
    public static void menuGoster(){
        ...
        System.out.print("Seçmek istediğiniz işlemin numarasını giriniz: ");
        ...
        if(i==1){
            ...
            k.adSoyad=adSoyad;
            System.out.print("Bağış Yapacak Kullanıcı ise 'E', değilse(Kan Alacak Kullanıcı ise) 'H' yazınız: ");
            scan = new Scanner(System.in);
            String bagisMi;
            while(true){
                bagisMi = scan.next();
            ...
            System.out.print("Kan Grubu faktörünü (harfini) yazınız: ");
            scan = new Scanner(System.in);
            String kanGrubu = scan.next();
            k.kanGrubu=kanGrubu;
            System.out.print("Kan Grubunun RH değerini (+ veya -) giriniz: ");
            scan = new Scanner(System.in);
            String RH = scan.next();
            k.RH=RH.charAt(0);
            System.out.print("HIV hastalığı varsa 'E', yoksa 'H' yazınız: ");
            scan = new Scanner(System.in);
            String HIV;
            while(true){
                HIV = scan.next();
            ...
            System.out.print("Hepatit B hastalığı varsa 'E', yoksa 'H' yazınız: ");
            scan = new Scanner(System.in);
            String hepatitB;
            while(true){
                hepatitB = scan.next();
            ...
            if(HIV.equals("E") || hepatitB.equals("E")) { //HIV veya hepatit varsa bagıscı olamaz
                if (bagisMi.equals("E")) {
                    System.out.println("Üzgünüz.. Hastalığınız olduğu için Kan Bağışçısı olamazsınız. Sadece Kan Alıcısı olabilirsiniz. " +
                            "Sisteme kaydınız Kan Alıcısı olarak oluşturulacaktır.");
                    k.bagisciMi = false;
                    k.aliciMi = true;
                }
            }
            ...
            System.out.println("Kan bağışı yapacak kişinin Ad Soyad bilgilerini giriniz:");
            scan = new Scanner(System.in);
            String kisi1=scan.next();
            System.out.println("Yapılacak bağış için Kan Alıcısının Ad Soyad bilgilerini giriniz:");
            scan = new Scanner(System.in);
            ...
            for(int y=0; y<kullaniciSayisi; y++){
                if(kullanicilar.get(y).adSoyad.equals(kisi1)) {
                    if (kullanicilar.get(y).bagisciMi) { //ilk girilen kullanıcı bagıscı mı
                        ...
                        System.out.println("Girilen ilk kullanıcı bağışcı olmadığı için bağış işlemi gerçekleştirilememektedir.");
                        islemDevam=false;
                    }
                }
                if(kullanicilar.get(y).adSoyad.equals(kisi2))
                    if(kullanicilar.get(y).aliciMi) { //ikinci girilen kişi alıcı mı
                        ...
                        System.out.println("Girilen ikinci kullanıcı kan alıcısı olmadığı için bağış işlemi gerçekleştirilememektedir.");
                        islemDevam = false;
                    }
            }
            if(islemDevam){ //kan bagıscı ve kan alıcı kişilerinde uyum varsa işlem gerçekleşir
                ...
                if(kan_iliski_matrisi[index1][index2]==1) {     //kanbagıs çizgesinden bagıs işlemi için kontrol yapılır
                    ...
                    System.out.println(kisi1+" kişisi "+kisi2+" kişisine kan veremez. Kan Bağış işlemi başarısız.");
            ...
            for (int k = 0; k < kanBagisCizgesi.size()-1; k=k+2) //ilk kullanıcı veren ikinci kullanıcı alan oldugu için 2şer arttırım yapıldı
